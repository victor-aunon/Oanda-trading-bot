...
class SaveResults:
    ...
    def save_optimization_results(
        ...
        for pair in pairs:
            ...
            pl_stats[f"Won {pair}"] = (len(pr_long) + len(pr_short))
            pl_stats["Lost"] += (len(lo_long) + len(lo_short))
            pl_stats[f"Lost {pair}"] = (len(lo_long) + len(lo_short))
            ...
            pl_stats[f"Trades {pair}"] = pl_stats[f"Won {pair}"] \
                + pl_stats[f"Lost {pair}"]
            pl_stats[f"Returns {pair}"] = (sum(pr_long) + sum(pr_short))
            pl_stats[f"Returns {pair}"] += (sum(lo_long) + sum(lo_short))
    ...
    def save_backtest_results(
        self, strategy_name: str, pair: str, trades: Dict[str, List[TradeType]]
    ) -> str:
        # Create xlsxwriter workbook
        summary_file = os.path.join(
            self.path,
            (
                f"Backtest_{pair}_{strategy_name}_"
        ...
        worksheet.add_table(
            0, 0,
            len(trades[pair]),
            len(trades[pair][0].keys()) - 1,
            {
                "columns": [{"header": col} for col
                            in trades[pair][0].keys()]
            }
        )
        for i, trade in enumerate(trades[pair]):
            worksheet.write_row(
                i + 1, 0, trade.values(),
